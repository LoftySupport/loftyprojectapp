...
| 15 Sep | Follow-up to decision 8, asked while building `0125`: does *"all notifications off by default"* include in-app, or only email, Teams and SMS? | **"External off, in-app stays on."** Given three options (everything off, in-app included, which was the recommendation because her own 25 toggles are all off; external off with in-app on; leave the defaults). So every notification type keeps in-app in its defaults and loses email, Teams and SMS (eight of fifteen carried email); a person opts in to an external channel in User settings, and a manager can put one back on a type in Setup. Her own preference rows stay as she set them: a choice already made is not a default. Built as the last statement of `0125`, with the proof asserting both halves |
