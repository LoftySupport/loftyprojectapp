...
| 11 Sep | Do Inbox and Tasks carry a count? | **Yes, both** — *"Inbox and tasks should have a badge"*. Three options were put up: leave both bare, Tasks only, or both with Inbox reading unread notifications; Amber took the last, and the cost named with it was that the number then appears twice, once beside Inbox and once on the bell. That cost is paid by making it **one number read once**: `InboxProvider` holds the two reads, the two-minute poll and the three mark-as-read calls, and the rail and the bell both render from it — so Mark all read clears both in the same paint, watched happening and watched failing with the rail reading separately. **Inbox** is unread notifications plus the tracker's moved requests, red, capped at 9+ like the bell. **Tasks** is `railCounts().myOpenTasks` — assigned to you and neither Done nor Cancelled — in the plain grey the other destinations use. Red on Tasks was not taken: a dozen open tasks is an ordinary week, and a badge that is red every day is a badge nobody sees. It is also the one count that does **not** match the screen it opens (My tasks lists the ticked-off ones too), which `RailCounts` records rather than leaves to be discovered by counting rows |
