...
| `addresses.consolidated_address` | Full address | The whole address as one string, assembled in the database so every card, export and search reads exactly the same text. | `text` | Not null. Maintained by the addresses_build_consolidated trigger, never written by the app — not a generated column, because a generation expression must be IMMUTABLE and the enum-to-text casts are not (enum_out is STABLE). Built with \|\| and coalesce rather than concat_ws, so a null part drops its separator. | Derived from street_2, lot_number, street_number, street_1, suburb, state, postcode, country — the lot number and postcode joined in 0025. Covered by a GIN trigram index, so search is fuzzy across the whole string. Read by project_display.current_address / original_address. | Created | 2026-08-01 · Amber Beaumont | 2026-08-01 · Amber Beaumont |
...
| `profile_teams.team` | Team (merged) | Merged into profiles.teams as the element type. The enum did the constraining, not the foreign key, so an array of it keeps exactly the integrity this column had. | `enum` | Dropped with the table in 0022. Replaced profile_teams.team_id in 0004, which was a uuid pointing at a teams table that was never built. | Superseded by profiles.teams. | Merged | 2026-08-01 · Amber Beaumont | 2026-08-01 · Amber Beaumont |
| `profile_teams.is_primary` | Primary team (merged) | Merged, and carrying no information when it went: true on all forty-five rows. It was meant to answer the questions needing one team — the dashboard panel, the default board filter — but a column with one value cannot answer anything. If a primary team is genuinely wanted, it needs to be a column somebody sets, not a flag derived from array position. | `boolean` | Dropped with the table in 0022. | No replacement. profiles.teams is an unordered set. | Merged | 2026-08-01 · Amber Beaumont | 2026-08-01 · Amber Beaumont |
...
| `profiles.teams` | Teams | The teams this person sits in. An array of the team enum since 0022, which folded in the profile_teams join table — that table held forty-five rows for forty-five people, nobody was in two teams, and is_primary was true on every row. | `enum` | team[]. Not null, default '{}'. Sorted, deduplicated and null-stripped by the profiles_normalise_teams trigger on write. GIN indexed. | Replaced the profile_teams table in 0022. Read by the Admin list and the user dialog; the 'team' and 'team_hierarchy' permission scopes will read it with the array overlap operator. | Created | 2026-08-01 · Amber Beaumont | 2026-08-01 · Amber Beaumont |
