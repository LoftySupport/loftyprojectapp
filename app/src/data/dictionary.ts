...
export const DICTIONARY: DictionaryEntry[] = [
  ...
  e("report_documents.report_document_share_token", "Share link",
    "A way into this document for somebody with no login. INERT AS SHIPPED: nothing in the app writes it, because reading by token needs the report-share endpoint and that is not deployed.",
    "text", "Nullable. UNIQUE. Partial index on the non-null rows. CHECK: both-or-neither with the expiry.",
    "Deliberately not served by RLS: making this table readable by `anon` to answer a share link would expose every other document in it to anybody holding any link. It has to be a server endpoint with the service role returning one row and only the viewer-safe parts of it.",
    "created"),
  ...
  e("report_documents.report_document_share_password_hash", "Link password",
    ...
    "Hashed server-side by the report-share endpoint when it is deployed. A hash computed in the browser protects nothing, and a hash returned to the browser is one somebody can attack offline.",
    "created"),
