...
export const DICTIONARY: DictionaryEntry[] = [
  ...
  e("profiles.teams", "Teams",
    "The teams this person sits in. An array of the team enum since 0022, which folded in the profile_teams join table — that table held forty-five rows for forty-five people, nobody was in two teams, and is_primary was true on every row.",
    "enum", "team[]. Not null, default '{}'. Sorted, deduplicated and null-stripped by the profiles_normalise_teams trigger on write. GIN indexed.",
    "Replaced the profile_teams table in 0022. Read by the Admin list and the user dialog; the 'team' and 'team_hierarchy' permission scopes will read it with the array overlap operator.",
    "created"),
  ...
  e("profile_teams.profile_id", "Person (merged)",
    ...
    "merged"),
  e("profile_teams.team", "Team (merged)",
    "Merged into profiles.teams as the element type. The enum did the constraining, not the foreign key, so an array of it keeps exactly the integrity this column had.",
    "enum", "Dropped with the table in 0022. Replaced profile_teams.team_id in 0004, which was a uuid pointing at a teams table that was never built.",
    "Superseded by profiles.teams.",
    "merged"),
  e("profile_teams.is_primary", "Primary team (merged)",
  ...
  e("address_history.id", "History entry ID",
  ...
  e("address_history.project_id", "Project",
  ...
  e("address_history.job_id", "Job",
    ...
    "created"),
  e("address_history.address_id", "Address",
    ...
    "created"),
  e("address_history.role", "Which address it was",
    ...
    "—", "created"),
  e("address_history.valid_from", "Applied from", "When this address started applying to the record.",
    "timestamptz", "Not null. CHECK address_history_period: valid_to >= valid_from.", "—", "created"),
  e("address_history.valid_to", "Applied until",
  ...
  e("address_history.changed_by", "Changed by", "Who repointed the record to a different address.",
    "uuid", "Nullable — a change made by an import or a migration has no person behind it.",
    "FK → profiles(id).", "created"),
  e("address_history.created_at", "Recorded on", "When the history row was written.",
  ...
  e("teams.id", "Team (merged)",
    ...
    "Referenced now as an enum value by profiles.teams, and by jobs.owning_team_id, property_defs.owning_team_id and template_phases.owning_team_id when those are built.",
    "merged"),
