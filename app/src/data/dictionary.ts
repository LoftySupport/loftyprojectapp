...
export const DICTIONARY: DictionaryEntry[] = [
  ...
  e("job_display.job_calculated_completion_missing", "Estimates missing",
    ...
    "created"),
  e("jobs.job_number_display", "Job number (as read)",
    "The job number a person reads: the job_id with a trailing \"c\" when the job is community title. Torrens reads without one, and so does a job whose title type nobody has set — which is the absence of a claim, not a claim that it is Torrens. Amber, 14 September: \"Any job that is listed as community title needs a 'c' suffix after the job number eg 1004-001c. Torrens title has no suffix. The jobs remain sequential.\"",
    "generated text",
    "GENERATED ALWAYS AS (job_id || case when job_title_type = 'community' then 'c' else '' end) STORED. Cannot be written to, and follows job_title_type on its own.",
    "NOT the key. job_id is, and it does not carry the suffix: the title type stays editable during a build, 67 of 83 live jobs have none set, and 17 tables point at job_id — one of them, report_documents, without ON UPDATE CASCADE. Amber chose the suffix on the displayed number over a suffix in the key so that correcting a title type cannot renumber a job that is already written into a contract, an email or a SharePoint folder. Route and look up by job_id; print this. 0120.",
    "created"),
