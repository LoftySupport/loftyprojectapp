...
export const DICTIONARY: DictionaryEntry[] = [
  ...
  e("jobs.project_no", "Project number",
    "The friendly project number, user-facing. Held on the job only so job_name can be a generated column — a generated column cannot reach another table. Never written by the app.",
  ...
  e("jobs.job_number", "Job number",
    "The sequence within the project — 01, 02, 03. Allocated automatically: insert a job without one and a trigger assigns the next.",
    "text", "Not null. Unique with project_id. Zero-padded to two digits, and wider than two past 99 rather than truncating.",
    "Assigned by the jobs_assign_number trigger, which locks the parent project row first — two concurrent inserts would otherwise read the same max and collide on the unique index. Feeds job_name.",
    "created"),
  e("jobs.job_name", "Job name",
    "What people quote and say out loud — '1001-01'. The project number and the job number joined, so the parent reads straight off the child.",
    "generated text", "GENERATED ALWAYS AS (project_no::text || '-' || job_number) STORED. Unique.",
    "Derived from jobs.project_no + jobs.job_number. Exposed by job_display.job_name.",
    "created"),
  ...
  e("job_display.job_name", "Job name",
    "The job's quotable name, joined for the board and for search.",
    "view", "Read-only.", "jobs.job_name.", "created"),
