...
export const DICTIONARY: DictionaryEntry[] = [
  ...
  e("profiles.profile_permission", "Permission level",
    ...
    "created"),
  e("profiles.profile_job_title", "Job title",
    "The person's title, shown on their profile. Not a lookup, and not tied to permission — a Manager by title may be a user by permission, and the two are unrelated.",
    "text",
    "Nullable. SYNCED FROM AZURE (Microsoft Entra), not owned by this app: a correction made here is silently overwritten the next time the person signs in. Change it in Entra. Recorded because the title and the team can legitimately disagree — Olivia Sinderberry is a Sales Administrator in the Acquisition & Development team — and the natural reaction to that is to 'fix' the wrong one of the two.",
    "Team membership is profile_teams, and it is what permissions read. The title is descriptive only.",
    "created"),
  ...
  e("addresses.address_street_number", "Street number",
    ...
    "text",
    "Nullable. Was half of \"a street needs one of the two numbers\"; that check went in 0073 and the rule now applies only to jobs, through guard_job_address_is_a_street. TEXT, and 0110 is where that stopped being an inherited assumption and became a measured fact: 12 of 178 are not numbers — 2-4, 42-44, 60-62, 84-88, 337-339, 3&5, 4-11/9, 1a, 2A, 4a, 83a. Amber, 10 Sep: \"a street number can be something like 100-105 (as text) or 12B\". Trimmed on write since 0110, after one row was found stored as \"13 \" and rendering \"13  Awoonga Road\".",
  ...
  e("tasks.maintenance_request_id", "Maintenance issue", "The maintenance issue this task is the work for, or empty for ordinary work.",
    "uuid", "Nullable. References maintenance_requests together with job_id, so the pair must name a real issue on that job.",
    "Added by 0120, and NOT a parent: tasks_one_parent still requires exactly one of job_id and project_id, because the Tasks board reads by job and a task whose only parent was an issue would vanish from it. The composite foreign key is what stops a task on one job carrying another job's issue, which would put a repair to one house on another house's board.",
    "created"),
  ...
  // ----------------------------------------------------------------- variations
  e("variations.variation_id", "Variation",
    "One change to a job. A record rather than a state on the job, because three teams raising conflicting changes at once is the problem this exists for, and a flag cannot represent three of anything.",
    "uuid", "Primary key.", "Referenced by variation_reopened_tasks.", "created"),
  e("variations.job_id", "Job", "The job being changed. Always a job, never a project: a project-level change is an ordinary edit, and project properties are read through by their jobs rather than copied.",
    "text", "Not null.", "FK → jobs(job_id) ON UPDATE CASCADE ON DELETE CASCADE.", "created"),
  e("variations.variation_sequence", "Variation number within the job",
    "1, 2, 3 — per job, so V3 on 1042-01 is unrelated to V3 on 1042-02.",
    "integer",
    "smallint. Not null, >= 1, unique with job_id. Allocated from jobs.job_variation_seq_high_water under a row lock — a high-water mark rather than max()+1, because a deleted V3 must never be reissued when that number has been in an email to a client.",
    "—", "created"),
  e("variations.variation_number", "Variation reference", "'1042-01-V3'. What appears in client correspondence.",
    "text", "Not null. Unique. Stamped at insert, and CHECK variations_number_matches_its_parts keeps it equal to job_id || '-V' || sequence, so it can never drift from them.",
    "Recomputed by variations_resync_number only when the parent job is renumbered.", "created"),
  e("variations.variation_title", "Title", "What the change is.", "text", "Not null, not blank.", "—", "created"),
  e("variations.variation_reason", "Reason",
    "Why it was raised, captured at the point of raising. Lofty asked for this specifically: a variation without its reason is an argument nobody can settle six months later.",
    "text", "Nullable.", "—", "created"),
  e("variations.variation_origin", "Raised by whom",
    "client · lofty · consultant · authority · supplier. A client-requested change and a Lofty-caused rework are the same shape and completely different facts, and the difference decides who pays.",
    "text", "Not null, default 'client'. CHECK on the five values.", "Grouped by in variation_rework.", "created"),
  e("variations.variation_status", "Status",
    "new → with_us → waiting_on_external → waiting_on_client → on_hold → completed | cancelled. Lofty's own words.",
    "text", "Not null, default 'new'. Text with a check rather than an enum, because this is a process and processes change. CHECK variations_completed_is_approved ties completed to an approval date.",
    "Drives the partial indexes and the job badge.", "created"),
  e("variations.variation_current_team", "With whom",
    "Which team is holding it right now — the board reads \"With us — Estimating\". Not the job's owning team: the point of a variation is that it moves between teams while the job stays where it is.",
    "text", "Nullable.", "FK → teams(team_id) ON UPDATE CASCADE. Partially indexed on the open statuses.", "created"),
  e("variations.variation_assignee_id", "Assignee", "The person handling it.", "uuid", "Nullable.", "FK → profiles(profile_id).", "created"),
  e("variations.variation_cost", "Cost",
    "What the change is worth. Nullable, because a variation is raised long before it is priced.",
    "numeric", "numeric(12,2), never a float — money that does not add up exactly is money somebody argues about.",
    "Summed by job_variation_summary for APPROVED variations only: a proposed change is not money spent.", "created"),
  e("variations.variation_days_impact", "Days impact", "How much time the change adds.", "integer", "smallint. Nullable.", "Summed for approved variations in job_variation_summary.", "created"),
  e("variations.variation_raised_at", "Raised on", "When it was requested.", "timestamptz", "Not null, default now().", "—", "created"),
  e("variations.variation_raised_by", "Raised by", "Who logged it.", "uuid", "Nullable.", "FK → profiles(profile_id).", "created"),
  e("variations.variation_approved_at", "Approved on",
    "When it was approved. Stamped by the database when the status becomes completed, never sent by the client — this one has a dollar value attached.",
    "timestamptz", "Nullable. Required once the status is completed.",
    "Paired with the approver by CHECK variations_approved_by_needs_a_time.", "created"),
  e("variations.variation_approved_by", "Approved by",
    "Who approved it. MAY be null while the date is set — \"approved on this date, by somebody we no longer know\" is a real state for history imported from the old system. The reverse is forbidden: an approver with no approval date is meaningless.",
    "uuid", "Nullable. CHECK variations_approved_by_needs_a_time.",
    "FK → profiles(profile_id). Falls through to the support profile when an automation approves, the way created_by does.", "created"),
  e("variations.variation_cancelled_reason", "Cancelled because",
    "Why it was dropped. CHECK-enforced rather than left to convention: the reason is the only thing that makes a cancelled variation worth keeping instead of deleting.",
    "text", "Nullable, but not null and not blank once the status is cancelled.", "—", "created"),

  // ------------------------------------------------------ variation_reopened_tasks
  e("variation_reopened_tasks.variation_id", "Variation", "The change that sent work back.",
    "uuid", "Part of the primary key.", "FK → variations(variation_id) ON DELETE CASCADE. A trigger refuses a task on a different job.", "created"),
  e("variation_reopened_tasks.task_id", "Task", "The task that had to be done again.",
    "uuid", "Part of the primary key. Indexed on its own.", "FK → tasks(task_id) ON DELETE CASCADE.", "created"),
  e("variation_reopened_tasks.variation_reopened_task_was_complete", "Was already finished",
    "Whether the task was already done when the variation reopened it. THIS is the number that makes a process argument settleable — work that was finished, and then had to be finished again.",
    "boolean",
    "Not null. SNAPSHOTTED by the trigger at the moment the row is written, not derived. Derived would be wrong twice over: the task is about to be reopened, so its live state no longer describes what the change interrupted, and a second variation reopening the same task would overwrite the first one's record.",
    "Counted by variation_rework.tasks_that_were_finished. Cannot be backfilled — a reopened-and-refinished task just looks slow.", "created"),
  e("variation_reopened_tasks.variation_reopened_task_completed_at", "Had been finished on",
  ...
  e("documents.document_storage_path", "Storage path", "Where the bytes live in Supabase Storage. Nullable, so a row can exist for a document Lofty expects but has not received — \"the signed contract\" as an outstanding item is a real state.",
    ...
    "One of three states with document_url: a path and no URL is an upload, a URL and no path is a pointer, neither is a document that has not arrived. It is the presence of a PATH that stops 0103's reaper deleting a row when its last link goes.",
    "created"),
  e("documents.document_url", "Where it lives",
    "The document's address when Lofty does not hold the bytes — a SharePoint link, pasted by hand until the integration lands. Amber, 10 Sep: \"when adding a document I need to be able to save it as a url in sharepoint (integration coming) but for now I need to be able to add and delete them\".",
    "text", "Nullable. CHECK documents_url_is_https — https and no whitespace, deliberately loose. UNIQUE where not null (documents_one_row_per_url).",
    "The app never fetches it: the link opens under the reader's own Microsoft session, so filing one is not a way of sharing a file — somebody without access sees SharePoint's refusal. Loose on purpose, because tenant paths, personal sites and shortened share links all look different and a pattern tight enough for one refuses the other two. Unique because one SharePoint address is one document, which is what makes filing the same contract on a project and on its job a second ATTACHMENT rather than a second copy. Independent of document_storage_path, and not exclusive with it — a synced document will have both.",
    "created"),
  ...
  e("comments.maintenance_request_id", "Maintenance issue", "The maintenance issue this comment is on — the sixth parent, and a comment has exactly one.",
    "uuid", "Nullable. References maintenance_requests. Cascades on delete.",
    "Added by 0120 when Amber chose to join the general tables rather than render the issue's own maintenance_messages in the comments panel. Separate from maintenance_messages, which stays the record of what was SENT to a contractor or a homeowner; this is Lofty talking to itself about the defect, with the @mentions, bell and edited marker the general thread already has.",
    "created"),
  ...
  e("activity_events.maintenance_request_id", "Maintenance issue", "The maintenance issue this event is about.",
    "uuid", "Nullable. References maintenance_requests. Cascades on delete.",
    "Added by 0120. Until then an issue kept no history of who changed what — a repair that changed hands or a booking that moved twice left nothing behind, which is the thing people argue about afterwards.",
    "created"),
  ...
  e("projects.project_name", "Project name",
    "What the project is called: the number, then the suburb and street — 1042 - REYNELLA, 14 Brodie Road (Amber, 28 Aug). Composed, not typed: the create form's name box has gone.",
    "text", "Nullable. Written by the repository immediately after the insert, once the sequence has issued the number.",
    "Free text produced a different convention on every project — \"14 Brodie Road, Reynella\", \"Howard Street Windsor Gardens\", \"St Clair 2007 St Clair Ave\" — none of them sortable and none carrying the number everything else is filed under. Composed from the CURRENT address, so it says where the project is; the six projects created before this still carry their typed names and have not been rewritten.",
    "created"),
  e("projects.project_proposed_dwellings", "Proposed dwellings",
    "How many lots were intended when the project was created — now the TOTAL, with the split held in the two columns below (0053). A different fact from how many jobs exist, which is why it is stored while the count is not: \"we planned four lots and got three\" needs both numbers, and a derived count can only ever tell you the second.",
    "integer", "smallint. Nullable. CHECK (> 0). CHECK project_lot_split_adds_up: when this and both splits are set, this must equal their sum.",
    "The form has a Total lots box (Amber, 31 Aug), which follows the split until somebody types over it — so the usual value is still the sum, and a total on its own is the case this column outlived 0053 for: an old-system number with no split recorded against it. The actual count is count(*) over jobs.project_id — derived, never stored. The CONFIRMED count is that same count excluding cancelled jobs.",
    "created"),
  e("projects.project_community_title_lots", "Community title lots",
    "How many of the proposed lots are community title (Amber, 28 Aug: \"these are different types… so now where you set 6 lots, 3 may be community title, and 3 may be torrens title and we need to know that split\").",
    "integer", "smallint. Nullable, CHECK (>= 0). Part of project_lot_split_adds_up.",
    "Null means the split is not known — true of every project created before the column, and NOT the same statement as zero. Zero is allowed and means none of them are community title. Seeds each row's title type in the split dialog.",
    "created"),
  e("projects.project_torrens_title_lots", "Torrens title lots",
    "How many of the proposed lots are Torrens title. The other half of the split; the total is these two added together.",
    "integer", "smallint. Nullable, CHECK (>= 0). Part of project_lot_split_adds_up.",
    "The constraint only bites when the total and both splits are all present, so a legacy row — a total with an unknown split — is left alone rather than made illegal.",
    "created"),
  ...
  e("projects.project_current_address_id", "Current address",
    "What every card, board and search shows. Identical to the original until something changes. Since 0118 moving it also moves the project's live jobs, where a job was still standing at the address the project is leaving — each keeping any lot and res number of its own, and a job re-addressed on its own since left alone.",
  ...
  e("projects.project_stage_entered_at", "Entered this phase",
    ...
    "timestamptz", "Not null, default now(). Moved by a trigger whenever project_stage changes.", "—", "created"),
  e("projects.project_sharepoint_url", "SharePoint folder",
    "The project's folder. Each of its jobs has a subfolder inside it, held on the job — so a project with three jobs shows four links, one per record rather than a list on one of them.",
    "text", "Nullable. CHECK projects_sharepoint_url_is_https — https and not blank. Deliberately loose: tenants, personal sites and shortened links all look different.",
    "Read alongside jobs.job_sharepoint_url on a job's record page.", "created"),
  ...
  // ------------------------------------------------------------ project_display
  e("project_display.project_current_address", "Project address (current)",
    "The consolidated current address, joined for the cards. A view because a generated column cannot reach another table.",
    "view", "Read-only.", "projects ⋈ addresses on project_current_address_id.", "created"),
  e("project_display.project_original_address", "Project address (original)",
    "The consolidated original address, for paperwork and search.",
    "view", "Read-only.", "projects ⋈ addresses on project_original_address_id.", "created"),
  e("project_display.project_council", "Council region",
    ...
    "view", "Read-only. sa_council.", "Reads addresses.address_council via project_current_address_id. Replaced project_display.council_id.", "created"),
  ...
  e("jobs.job_sequence", "Job sequence",
    "The counter within the project — 001, 002, 003. Allocated automatically: insert a job without one and a trigger assigns the next. NOT what Lofty calls the job number — that is jobs.job_id, the combined value.",
    "text", "Not null. Unique with project_id. Zero-padded to THREE digits since 0073 (Amber: \"all three digits and start at 001 … we can have over 100 jobs on a project\"), written plainly past 999, and enforced by CHECK jobs_sequence_is_padded rather than left to the trigger — the app never supplies a sequence but the import writes rows directly, and an unpadded \"2\" beside an existing \"002\" is two jobs a person reads as one. CHECK (>= 1) — there is no zeroth job. Padding to two was what capped a project at 99 jobs: lpad truncates, so job 100 was handed job 10\u2019s sequence and the key refused it.",
  ...
  e("jobs.job_id", "Job number",
    "The job number as Lofty uses the phrase — '1042-001' — and the primary key. What people type, quote and say out loud is what the database joins on.",
    "text",
    "Primary key. STAMPED by the jobs_assign_sequence trigger at insert from project_id, job_sequence and — since 0120 — job_title_type, which appends a 'c' on a community title job. Torrens and not-yet-decided carry no suffix. jobs_id_matches_its_parts holds the shape; job_number(project, sequence, title_type) is the one definition the triggers build from.",
    "Recomputed by jobs_resync_job_id whenever a part of it changes: the project, the sequence, or since 0120 the title type. Amber, 14 September: \"can it be updated that is also linked so it show the c on the end (like the address when updated) but the project 4 digits and 3 digit job code always remains with job too\" — so 1004-003 becomes 1004-003c and back, and can never become 1004-004. Seventeen foreign keys into it are ON UPDATE CASCADE and a rename carries every child row with it; report_documents was the seventeenth, made to cascade by 0120 because it would otherwise have refused the first rename. Also widened once by 0073 from two digits to three.",
    "created"),
  ...
  e("jobs.job_title_type", "Title type",
    ...
    "text", "Nullable. CHECK: community | torrens.",
    "Set at the split from the project's mix — first the community lots, then the Torrens ones — and editable per job in the drawer, because nothing says which lots take which title. Null means nobody has said, which is every job created before the column; a default would have put a title type on 60 jobs nobody chose. Text under a CHECK rather than an enum, the same call 0035 made for the lifecycle.",
    "created"),
  e("jobs.job_target_completion", "Completion date (target)", "The date this job is being worked towards — the sixth key property on the job record. Amber, 11 September: \"each job has its own completion date… there is also a project completion level which is when all jobs in the project are completed\".", "date", "Nullable.", "Mirrors projects.project_target_completion. The two pairs are SEPARATE and relate rather than one deriving the other — \"project date and job dates are separate and [it] depends [on] each other\". A new job's target is deliberately NOT seeded from its project's: a job with no target says so rather than inheriting a date nobody set for it.", "created"),
  e("jobs.job_end_date", "Completion date (actual)", "The day the job actually finished, as opposed to the date aimed at. Null until it has. The job record shows the target while the job runs and this once it is done, which is what 6b draws.", "date", "Nullable. CHECK: not before the job was created.", "Both dates exist because collapsing them loses the distinction the moment a job finishes on a different day from the one planned, which is most jobs. A PROJECT'S completion is not a column anywhere: it is derived from all of its jobs being completed, and storing it would be a second answer that can go stale.", "created"),
  e("job_display.job_calculated_completion", "Completion date (calculated)",
    "What the SLAs say the job will finish, beside the target somebody committed to and the day it actually did. Amber, 14 September: a system field \"based by when the job is likely to end based on slas and [the stage] it is up to so management can look at targeted completion date (when they want it to be done) versus the realistic calculated date based on slas and then the actual date it was completed for process optimisation\".",
    "date",
    "Read-only, computed. NULL is the normal answer today and job_calculated_completion_missing says why — all 38 Pre-construction processes carry no estimate, so the forecast refuses to answer rather than projecting from the third of the pipeline that is populated. NULL for both columns means the job is not live.",
    "The longest path through process_dependencies in CALENDAR days, from job_completion_forecast() (0119). A process costs its own process_expected_days when set and the sum of its task steps' process_step_expected_days otherwise — 3 of 51 processes use the first, 107 of 107 task steps feed the second (0131 moved it off the template table). Every start is floored at today, so an overrun is sunk rather than pushed forward. Counts every process on the path, optional or not: 0127 added process_is_optional and the forecast does not read it yet. It is Stage 2's completion gate that will, and the forecast follows it there.",
    "created"),
  e("job_display.job_calculated_completion_missing", "Estimates missing",
    "How many of the job's processes nobody has put a duration on. It is why the calculated completion date is blank when it is blank.",
    "integer",
    "Read-only, computed. 0 when the forecast is real. NULL when the job is not live.",
    "From job_completion_forecast() (0119). Exists so an empty cell is a number somebody can act on rather than a mystery — the repository's rule that a blank invites configuring while a guess gets quoted back as agreed.",
    "created"),
  ...
  e("jobs.job_current_address_id", "Current address",
    "What every card, board and search result shows. Identical to the original until something changes — a lot renumbered by council, a street renamed, a typo found at handover. Set from the job's drawer, and also by the project: since 0118, repointing the project's current address carries every live job that was still standing at the old one, keeping the job's own lot and res numbers.",
    "uuid", "Not null. Falls back to the original in a trigger. Indexed. Also written by jobs_follow_project_address (0118) when the parent project's current address repoints.",
  ...
  e("jobs.job_status", "Status",
    ...
    "created"),
  e("jobs.job_sharepoint_url", "SharePoint folder",
    "This job's own subfolder inside its project's folder. A job's record page shows this and the project's; it does not show its siblings'.",
    "text", "Nullable. CHECK jobs_sharepoint_url_is_https.",
    "The project's folder is read through the project rather than copied here — a copy would be a second place for it to be wrong the day a site moves.",
    "created"),
  ...
  e("job_address_search.address_role", "Matched address",
    ...
    "created"),
  e("project_address_search.address_role", "Matched address",
  ...
  e("activity_audit.activity_audit_maintenance_request_id", "Maintenance issue", "The maintenance issue this audit row is about, resolved the same way the job and the project are.",
    "uuid", "Nullable, and null for the great majority of rows. Indexed partially.",
    "Added by 0121, correcting 0120: that migration gave activity_events a maintenance parent, but nothing in the app reads activity_events — the Activity panel reads this table. Denormalised on purpose, because a record's history has to be an index lookup rather than a jsonb-path scan, which is what 0080 removed. It also catches rows about OTHER tables that belong to the issue: a comment, a task, a photo attached.",
    "created"),
  ...
  e("property_defs.property_def_group", "Group", "The group of work the property belongs to, under the workbook's own header: a Construction sub-stage (Footings, Frame, Roof Cover, External Cladding, 2nd Fix, Practical Completion, Handover) or a Pre-construction process (Working Drawings, Selections, SA Water and so on). Named property_def_automation from 0043 to 0124 and never held an automation; 0090 carried the words in and said so.", "text", "Nullable. Free text until Stage 1 gives sub-stages rows and Stage 2 gives processes steps; each takes its half.", "—", "created"),
  ...
  // -------------------------------------------------------------------- processes (0078)
  e("processes.process_id", "Process ID", "What happens inside a lifecycle stage, as a row: a named piece of work pinned to a stage, run on a project or a job, with a team, a duration and the properties it collects. Never stores a value — properties do.", "uuid", "Primary key.", "process_runs, process_dependencies and process_steps hang off it.", "created"),
  ...
  e("processes.process_is_optional", "Optional", "A process a job may skip (0127): its sub-stage can complete without it, and Stage 3's derivation never holds a sub-stage open for one. False for every process on the day. Amber, 15 September: \"Some entire processes need to be marked as optional and they can be skipped.\"", "boolean", "Not null, default false. CHECK processes_a_milestone_is_never_optional (0138) — never true at the same time as process_is_milestone.", "Stage 2's completion gate reads it; Setup → Processes disables the box on a milestone and says why.", "created"),
  ...
  e("processes.process_is_milestone", "Milestone", "Whether passing this process is a milestone of its stage. A count of these is the only progress figure the app reports — never a percentage, which is why it is never optional: the count means nothing if the total depends on what each job skipped.", "boolean", "Not null, default false. CHECK processes_a_milestone_is_never_optional (0138).", "The drawer's Milestones panel reads it; Setup → Processes disables the box on an optional process and says why.", "created"),
  ...
  e("process_steps.process_step_is_required", "Required", "A run cannot be marked complete while a required step is open (Stage 2 gate). Property steps carry the flag the old template list held; task steps arrived required, which is a reading of Amber's \"when all process steps are completed\" rather than a copy of anything.", "boolean", "Not null, default true.", "—", "created"),
  ...
  e("process_step_dependencies.process_step_dependency_lag_days", "Lag days", "\"Handover is 10 days after the PCI walkthrough\" — 93+10 in the Construction schedule.", "integer", "Not null, default 0. CHECK >= 0.", "Copied into task_dependencies.task_dependency_lag_days on instantiation.", "created"),
  ...
  e("tasks.process_step_id", "Step", "The task step this task was made from, for \"which jobs skipped the frame check\". Renamed from process_task_id by 0131, when process_tasks went; the values did not move.", "uuid", "Nullable. FK → process_steps ON DELETE SET NULL — survives the step being removed.", "—", "created"),
  ...
  e("company_display.company_primary_contact_name", "Primary contact", "The person you ring \u2014 the one flagged primary on company_contacts, while they still work there.", "view", "Null when nobody is flagged; the screen says so rather than promoting the longest-serving employee.", "\u2014", "created"),
  e("company_display.company_primary_contact_email", "Contact email", "That person's own email, not the company's.", "view", "\u2014", "Separate from Email above: a mailbox and a person are different facts.", "created"),
  e("company_display.company_primary_contact_phone", "Contact phone", "That person's own mobile or phone, not the switchboard.", "view", "Mobile preferred over landline.", "\u2014", "created"),
  e("company_display.company_primary_contact_role", "Contact role", "Their job there \u2014 site supervisor, estimator.", "view", "\u2014", "\u2014", "created"),
  e("company_display.company_suburb", "Suburb", "The suburb of the company's address, on its own.", "view", "\u2014", "Beside the whole address rather than parsed back out of it.", "created"),
  ...
  // 0114 — the header Amber types once, and the per-issue fields that follow it.
  e("maintenance_requests.maintenance_request_identified_on", "Date identified", "The day the issue was identified, as opposed to the moment the row was logged. Defaults to today in the drawer and can be cleared.", "date", "Nullable \u2014 clearable is half of what Amber asked for, so it cannot be not null.", "Distinct from Reported, which is still the SLA clock.", "created"),
  e("maintenance_requests.maintenance_request_identified_at", "Identified at", "Where it was found: PCI, Building Inspector (Client), Building Inspector (House Inspect), Handover Inspection, Site Inspection, 1/2/3 Month Inspection, Other.", "text", "Nullable. CHECK on Amber's nine (proved biting).", "A CHECK rather than a lookup table \u2014 nothing edits the list yet.", "created"),
  e("maintenance_requests.maintenance_request_reported_by_profile_id", "Reported by", "The Lofty person who reported it \u2014 Amber: \"change to select from Internal User name\".", "uuid", "Nullable. FK \u2192 profiles.", "Separate from Reported by (contact), which is the homeowner an email or form came from.", "created"),
  e("maintenance_requests.maintenance_request_batch_id", "Batch", "The issues typed into one drawer share this, so three defects from one PCI walk stay one sitting.", "uuid", "Nullable. No FK and no table \u2014 it records a fact, not a parent.", "The maintenance report groups a section on it.", "created"),
  e("maintenance_requests.maintenance_request_assignee_kind", "Assigned to", "Internal or external \u2014 the radio that decides which picker the drawer shows.", "text", "Not null, default internal. CHECK on the two (proved biting).", "CHECK: internal cannot name a company and external cannot name a person.", "created"),
  e("maintenance_requests.maintenance_request_assignee_profile_id", "Assignee (internal)", "The Lofty person fixing it, when the kind is internal.", "uuid", "Nullable. FK \u2192 profiles. Refused when the kind is external.", "Not an offer \u2014 maintenance_assignments is the offer, with its accept link.", "created"),
  e("maintenance_requests.maintenance_request_assigned_company_id", "Assignee (external)", "The trade or contractor fixing it, when the kind is external.", "uuid", "Nullable. FK \u2192 companies. Refused when the kind is internal.", "Typing a name nothing matches creates the company as a contractor; a manager still approves it.", "created"),
  e("maintenance_requests.maintenance_request_booked_on", "Date booked", "The day the repair is booked in.", "date", "Nullable.", "Per issue, which is why each issue is its own request.", "created"),
  e("maintenance_requests.maintenance_request_completed_on", "Date completed", "The day the repair was actually done \u2014 a date somebody sets, like the booking beside it.", "date", "Nullable. Nothing derives it and no trigger stamps it (0116).", "Not the status and not Closed: a request can carry this date while its status is still In progress, because the tradesperson finishing and the ticket being closed are two events.", "created"),
  e("maintenance_requests.maintenance_request_followup_on", "Follow-up", "The day to chase this issue.", "date", "Nullable. Nothing derives it.", "\u2014", "created"),
  ...
  e("feedback.feedback_kind", "Kind", "Bug or idea — something that went wrong, or something that could be better. The Setup tabs are this column filtered.", "text", "Not null. CHECK: bug | idea.", "One table rather than two: a bug and an idea are the same row with a different word on it, and two tables would mean two policies and two screens that drift apart.", "created"),
  e("feedback.feedback_title", "One line", "What the list shows — \"Saving a job did nothing\".", "text", "Not null. CHECK: not blank after trimming.", "A report nobody can identify in a list is a report nobody will action, so blank is refused at the door rather than in the form only.", "created"),
  e("feedback.feedback_detail", "Detail", "Everything else: what was expected, what happened instead. Empty is allowed and means empty.", "text", "Not null, default \'\'.", "Somebody with a title and nothing more is still worth having; requiring detail is a reason not to report at all.", "created"),
  e("feedback.feedback_page", "Where", "The app path they were on when they sent it — captured, never typed. For a bug this is most of the reproduction.", "text", "Nullable.", "The one field a person reporting in a hurry always leaves out, which is why the form takes it rather than asking. Shown in the form before sending, so it is not collected silently.", "created"),
  e("feedback.feedback_stage", "Stage", "Where the request sits in the queue everybody can see: requested, in review, planned, in development, shipped or declined. Renamed from feedback_status in 0060, when the list stopped being a private triage board.", "text", "Not null, default requested. CHECK: requested | in_review | planned | in_development | shipped | declined.", "Only superadmin may change it (Amber, 30 Aug), enforced by guard_feedback_stage_change() rather than by a policy — RLS decides rows, not columns, and the UPDATE policy has to stay at admin so admins can still fix a title. Declined rather than a delete: it keeps the record of having considered something.", "created"),
  e("feedback.feedback_stage_entered_at", "In this stage since", "When it reached the stage it is in — so \"stuck since June\" is answerable and not a guess.", "timestamptz", "Not null, default now(). Backfilled to the created time in 0060.", "Stamped by guard_feedback_stage_change(), never sent by the client: a date the sender chooses is not a record of when anything happened.", "created"),
  e("feedback.feedback_error_text", "Error", "The error the app was showing when it was sent — captured from the app's own error state, never typed.", "text", "Nullable.", "Nobody retypes an error message accurately in a hurry. Held for fifteen minutes and shown in the form before sending, so it is neither stale nor collected silently.", "created"),
  e("feedback.feedback_user_agent", "Browser", "The browser and version it was sent from.", "text", "Nullable.", "Captured because nobody knows their own browser version, and it is the first thing asked when a bug reproduces for one person only.", "created"),
  e("feedback.roadmap_phase_id", "Planned into", "Which roadmap phase this request is scheduled in, when it is scheduled at all. Null is the normal state.", "uuid", "Nullable. FK → roadmap_phases(roadmap_phase_id) ON DELETE SET NULL.", "SET NULL and never CASCADE: deleting a phase must not delete the requests planned into it, which is the most destructive plausible mistake on that screen.", "created"),

  // ------------------------------------------------------- the tracker (0061-0063)
  e("feedback_votes.feedback_id", "Request voted on", "Which request the thumbs up is on.", "uuid", "Part of the primary key. FK → feedback(feedback_id) ON DELETE CASCADE.", "Half of the primary key that IS the rule: (feedback_id, profile_id) cannot hold a second vote from the same person, whatever the app sends.", "created"),
  e("feedback_votes.profile_id", "Who voted", "The person whose vote it is.", "uuid", "Part of the primary key. FK → profiles(profile_id) ON DELETE CASCADE.", "NOT NULL and CASCADE, unlike feedback.profile_id: a report outlives its reporter because it is about the app, but a vote IS the person. Insert and delete policies both compare it to current_profile_id(), so nobody votes on anyone else's behalf or takes their vote away.", "created"),
  e("feedback_votes.feedback_vote_at", "Voted", "When the vote was cast.", "timestamptz", "Not null, default now().", "—", "created"),

  e("feedback_attachments.feedback_attachment_id", "Screenshot", "One file attached to a report (Amber, 30 Aug: \"and any screenshots\").", "uuid", "Primary key, default gen_random_uuid().", "The bytes live in the private feedback-screenshots storage bucket; this row is the only link between a file and the report it belongs to.", "created"),
  e("feedback_attachments.feedback_id", "On which report", "The report the screenshot belongs to.", "uuid", "Not null. FK → feedback(feedback_id) ON DELETE CASCADE.", "CASCADE, the opposite call to feedback.profile_id: a screenshot is part of the report rather than an independent fact about the app.", "created"),
  e("feedback_attachments.feedback_attachment_path", "Object path", "Where the file sits in the bucket — <profile_id>/<uuid>-<filename>.", "text", "Not null, unique. CHECK: not blank.", "The first path segment is the uploader, because the storage policy compares it to current_profile_id() — storage.objects has no column saying which report a file belongs to, and giving it one would be a second link that can disagree with this table.", "created"),
  e("feedback_attachments.feedback_attachment_name", "File name", "What the person called it on their machine — the link text, so three screenshots read as three named things.", "text", "Not null, default ''.", "Sanitised before it becomes part of an object path: a filename arrives from somebody's machine, and ../ in a path is the oldest trick there is.", "created"),
  e("feedback_attachments.feedback_attachment_mime", "Type", "image/png, application/pdf and so on.", "text", "Not null, default ''.", "Recorded rather than sniffed at render: asking the browser to guess is how a PDF gets drawn as a broken image.", "created"),
  e("feedback_attachments.feedback_attachment_bytes", "Size", "How big the file is.", "integer", "Not null, default 0. CHECK: not negative.", "The bucket refuses anything over 10MB, so integer is enough. Recorded so \"why is this report slow to open\" is answerable from a number rather than from a folder.", "created"),

  // ------------------------------------------- the Canny round (0064-0068)
  e("comments.feedback_id", "On which request", "The tracker request this comment is on — the fifth parent a comment may have.", "uuid", "Nullable. FK → feedback(feedback_id) ON DELETE CASCADE. Part of comments_one_parent: exactly one of project, job, task, variation or request.", "A fifth parent rather than a feedback_comments table, so tracker discussions get @mentions, the edited marker, the author stamp and the bell without any of them being built a second time.", "created"),
  e("comments.comment_is_pinned", "Pinned", "The official answer, held at the top of a thread.", "boolean", "Not null, default false.", "Admin+, enforced by guard_comment_standing() rather than by a policy: pinning is a COLUMN rule, and the author's own edit policy would otherwise let anybody pin themselves to the top. A boolean here rather than a pinned_comment_id on the parent — pinning is a property of the comment.", "created"),
  e("comments.comment_is_internal", "Internal", "The team's own lane: triage talk the person who asked cannot read.", "boolean", "Not null, default false.", "Enforced by the read policy, so an internal comment never reaches a non-admin at all — including the count on the board, which is why that count is computed in the view rather than in the app. Default false: nothing already written became hidden.", "created"),
  e("comments.comment_feedback_stage", "Stage announced", "Set when this comment was the note that came with a stage change — the stage it announced.", "text", "Nullable. CHECK: only on a comment that has a feedback_id.", "One note per MOVE rather than one per request: a column on feedback would mean the note explaining 'planned' was overwritten by the note explaining 'in development', and what was said at each step would be gone.", "created"),

  e("feedback.feedback_merged_into_id", "Duplicate of", "The request this one turned out to be a duplicate of. Null for everything that stands on its own.", "uuid", "Nullable. FK → feedback(feedback_id) ON DELETE SET NULL. CHECK: never itself.", "Merging moves the votes and the followers to the target, by a SECURITY DEFINER trigger — the app cannot, because a vote belongs to the person who cast it. Chains are refused, so a duplicate always points at a live request. The row is kept rather than deleted: the person who filed it must still be able to find it and see where the conversation went.", "created"),

  e("feedback_votes.feedback_vote_added_by", "Added by", "Who entered this vote, when it was not the voter — the request that arrived on a call or on site.", "uuid", "Nullable. FK → profiles(profile_id) ON DELETE SET NULL. CHECK: never equal to the voter.", "Admin+ by a second, narrower insert policy. The CHECK is not decoration: policies are OR'd, so the policy's own 'not your own vote' clause was silently ignored by the older own-vote policy — found by a probe asserting the refusal, not by reading the policy.", "created"),

  e("feedback_follows.feedback_id", "Request followed", "Which request somebody hears about.", "uuid", "Part of the primary key. FK → feedback(feedback_id) ON DELETE CASCADE.", "Voting and reporting both create one, by trigger — not by the app, because the same rows are written by an import, a merge and an on-behalf vote, and a follow created in the repository would exist for one of those and silently not for the others.", "created"),
  e("feedback_follows.profile_id", "Who follows it", "The person who hears about it.", "uuid", "Part of the primary key. FK → profiles(profile_id) ON DELETE CASCADE.", "Private, unlike a vote: your own follows are the only ones you can read. A vote is a position on a question everybody can see; following is closer to a bookmark.", "created"),
  e("feedback_follows.feedback_follow_seen_stage_at", "Seen at", "When this person last looked at where the request had got to. Null means never.", "timestamptz", "Nullable.", "Unread is DERIVED from this against feedback_stage_entered_at rather than stored as notification rows — so a notice cannot outlive, duplicate or contradict the move it describes, and there is nothing to clean up when a request is merged away. Null counts as unseen, so following something already in flight tells you where it is.", "created"),
  e("feedback_follows.feedback_follow_at", "Followed", "When they started following.", "timestamptz", "Not null, default now().", "—", "created"),

  e("roadmap_phases.roadmap_phase_id", "Phase", "One phase of the build, with the dates Amber shows people (30 Aug: \"a roadmap which is set to phases and dates so I can show how they are being developed\").", "uuid", "Primary key, default gen_random_uuid().", "Everybody reads; superadmin writes, the same bar as the lifecycle stages — deciding what the company is doing next is not the same act as asking for something.", "created"),
  e("roadmap_phases.roadmap_phase_name", "Name", "What people call it — \"Phase 2 — costings\".", "text", "Not null, unique. CHECK: not blank.", "—", "created"),
  e("roadmap_phases.roadmap_phase_summary", "What it covers", "A sentence under the heading. Empty allowed.", "text", "Not null, default ''.", "An empty summary is a different statement from an invented one; a phase named for its content may need no gloss.", "created"),
  e("roadmap_phases.roadmap_phase_starts_on", "Starts", "When the phase is intended to start. Null means it has not been scheduled.", "date", "Nullable. CHECK: ends_on >= starts_on when both are set.", "Nullable deliberately: an unscheduled phase is a real and common thing, and the roadmap draws it as undated rather than estimating a bar for it. A guessed date gets quoted back as a commitment.", "created"),
  e("roadmap_phases.roadmap_phase_ends_on", "Ends", "When it is intended to finish. Null means unscheduled.", "date", "Nullable. Same CHECK.", "Not > but >=: a phase can start and finish on one day.", "created"),
  e("roadmap_phases.roadmap_phase_position", "Order", "Where it sits in the sequence.", "integer", "Not null, unique index.", "Stored rather than derived from the dates, because an undated phase still has a place in the run. Unique so two phases cannot claim one slot; reordering renumbers through a parked position rather than swapping in place.", "created"),
  e("roadmap_phases.roadmap_phase_status", "Status", "Planned, in progress, or delivered.", "text", "Not null, default planned. CHECK: planned | in_progress | delivered.", "Asserted and never derived from the dates: a phase whose end date has passed is not thereby delivered, and a derived status would tell the whole company something shipped on the strength of a date set in June.", "created"),

  e("releases.release_id", "Release", "One thing that actually went out — the changelog people read.", "uuid", "Primary key, default gen_random_uuid().", "Everybody reads; superadmin writes. Separate from the roadmap because a plan that slips must never rewrite what the changelog said happened.", "created"),
  e("releases.release_version", "Version", "What people say out loud — \"0.9\", \"2026-08-30\".", "text", "Not null, unique. CHECK: not blank.", "Free text rather than a parsed semver: Lofty ships named and dated builds, and a type refusing that would be a schema deciding a naming convention nobody agreed to.", "created"),
  e("releases.release_name", "Name", "The headline — \"the tracker\". Empty allowed.", "text", "Not null, default ''.", "Some releases are a list of fixes and nothing more.", "created"),
  e("releases.release_summary", "Summary", "A paragraph under the heading.", "text", "Not null, default ''.", "—", "created"),
  e("releases.release_shipped_on", "Shipped", "The day it went out.", "date", "Not null, default current_date.", "A date and not a timestamp: nobody has asked what hour a release went out, and a timestamptz would make the answer depend on the reader's timezone.", "created"),

  e("release_entries.release_entry_id", "Changelog line", "One line under a release.", "uuid", "Primary key, default gen_random_uuid().", "—", "created"),
  e("release_entries.release_id", "In which release", "The release the line belongs to.", "uuid", "Not null. FK → releases(release_id) ON DELETE CASCADE.", "CASCADE: a line has no life without its release.", "created"),
  e("release_entries.release_entry_kind", "Kind", "Added, fixed, changed or removed.", "text", "Not null. CHECK: added | fixed | changed | removed.", "Keep a Changelog's four verbs and no more — they are also the vocabulary scripts/changelog.mjs parses out of commit trailers, so a fifth value would be one the tooling silently drops.", "created"),
  e("release_entries.release_entry_summary", "What changed", "The line itself.", "text", "Not null. CHECK: not blank.", "—", "created"),
  e("release_entries.feedback_id", "From which request", "The request this shipped, when there was one — how a person sees their own suggestion arrive.", "uuid", "Nullable. FK → feedback(feedback_id) ON DELETE SET NULL.", "Null for the improvements nobody filed a request for, which is most of them. SET NULL rather than CASCADE: deleting a request must never quietly remove a line from the history of what shipped.", "created"),
  e("release_entries.release_entry_position", "Order", "Where the line sits under its release.", "integer", "Not null, default 0.", "—", "created"),
  e("feedback.profile_id", "From", "Who sent it, so a report that needs a conversation has somebody to go back to.", "uuid", "Nullable. FK → profiles(profile_id) ON DELETE SET NULL.", "Stamped by the repository from the signed-in profile and checked by the insert policy against current_profile_id(), so nobody can file under a colleague\'s name. SET NULL rather than CASCADE: the report outlives the reporter — it is about the app, not the person who noticed.", "created"),
  e("feedback.feedback_added_by", "Entered by", "Who typed this request, when it was not the person it is from — the one that arrived on a call or on site.", "uuid", "Nullable. FK → profiles(profile_id) ON DELETE SET NULL. CHECK: never equal to profile_id.", "Admin+ by a second, narrower insert policy (0070). profile_id stays the person it is FROM, so \"Deanna asked for this\" and \"Amber says Deanna asked for this\" are different rows rather than the same one. The CHECK is not decoration: 0067 watched the identical policy clause be silently ignored on feedback_votes, because policies are OR\'d and the older own-row policy admitted the row first.", "created"),

  // ----------------------------------------------------------- pinned_pages (0112)
  e("pinned_pages.pinned_page_id", "Pinned page", "One page somebody has bookmarked into the navigation rail — Amber, 11 September: \"pinned is new and allows people to save/bookmark a page\". Any page: a filtered board, a settings screen, a job, a report.", "uuid", "Primary key, default gen_random_uuid().", "Private by RLS: the owner-only policy compares profile_id to current_profile_id(), so nobody sees anybody else's bookmarks.", "created"),
  e("pinned_pages.profile_id", "Whose pin", "The person the bookmark belongs to. Cascades on delete: a person's bookmarks are theirs and go with them.", "uuid", "Not null. FK → profiles(profile_id) ON DELETE CASCADE.", "Also the column the RLS policy filters on, and the leading column of both unique constraints that serve it.", "created"),
  e("pinned_pages.pinned_page_label", "Name", "What the person calls it in the rail — \"Wandi lots\", \"My overdue jobs\".", "text", "Not null. CHECK: not blank after trimming, and 60 characters or fewer.", "Capped because the rail ellipsises but a pasted 4 kB label is an accident rather than a name.", "created"),
  e("pinned_pages.pinned_page_url", "Page", "Where the bookmark goes — an in-app path with its query string, e.g. /jobs/1209-002 or /projects?saved=current. The kind of thing it points at is read off this path rather than stored, so the icon and the link can never disagree.", "text", "Not null. CHECK: starts with a single slash, 2000 characters or fewer. UNIQUE (profile_id, url).", "The leading-slash rule is a security constraint, not tidiness: this value is written by a person and rendered into an anchor's href in the one component on every screen, so an absolute or protocol-relative URL would navigate off Lofty from inside the navigation rail. Watched biting in the migration's own proof, both forms.", "created"),
  e("pinned_pages.pinned_page_position", "Slot", "Which of the five slots the pin occupies, and the order it draws in.", "integer", "Not null. CHECK: between 1 and 5. UNIQUE (profile_id, position).", "This pair is how \"max five\" is enforced — the sixth pin has nowhere to go because there is no sixth slot. A counting trigger was the alternative and is racy: two tabs pinning at once would both count four and both insert.", "created"),
  ...
  e("report_templates.report_template_name", "Name",
    ...
    "text", "Not null. CHECK: not blank after trimming. UNIQUE (kind, name).",
    ...
    "created"),
  e("report_templates.report_template_description", "What it is for",
    "The author's note about when to use it. Nullable, because not every template needs explaining and an empty string pretending to be a description is worse than a blank.",
    "text", "Nullable.", "—", "created"),
  e("report_templates.report_template_scope", "Who it is for",
    "A visibility band, not an owner: global (everybody), team (that team plus manager and above), managers (manager and above only). The author is report_template_created_by and never changes.",
    "text", "Not null, default 'global'. CHECK: global | team | managers.",
    "Enforced by the SELECT policy, not by the screen. Team scope reads profile_teams for an overlap, the same way every other team predicate in this schema does.",
    "created"),
  e("report_templates.team_id", "Which team",
    "The team a team-scoped entry belongs to. Required by exactly that scope and forbidden otherwise.",
    "text", "Nullable. FK → teams(team_id). CHECK: (scope = 'team') = (team_id is not null).",
    "Written as an equivalence rather than two checks: a team-scoped row with no team is invisible to everybody, and a global row carrying a team is a lie about who it is for.",
    "created"),
  e("report_templates.report_template_layout", "The layout",
    "The whole builder state as one document: { widgets: [{ id, kind, options }], page, theme }. A block holds the QUESTION — \"the jobs table grouped by stage\", \"the Site Start properties for this job\" — and never the answer, so a template opened in March renders March's jobs.",
    "jsonb", "Not null, default {\"widgets\": []}. CHECK: widgets is a JSON array.",
    "One column rather than a child table: nothing joins to a block, nothing filters by one, and the builder rewrites the whole list on every autosave. The CHECK is the shape only — a block's `kind` is registered in the app's widget adapter, so a constraint listing the kinds would need a migration for every new block. coalesce() inside it is load-bearing: jsonb_typeof of a missing key is NULL, and a CHECK reads NULL as a pass, so `= 'array'` alone accepted a layout with no widgets key at all.",
    "created"),
  e("report_templates.report_template_approved_at", "Signed off on",
    "When a manager put it in the library. Null means it is still the author's draft — and nobody else can see it, which is the read policy rather than the screen. Amber, 4 Sep: anyone may write a template, \"a manager and above must approve it before it is saved as a template in the template library\".",
    "timestamptz", "Nullable. CHECK: both-or-neither with report_template_approved_by.",
    "Stamped by guard_report_template_approval() from the session, never typed — the same shape, and the same error code, as guard_party_approval() on contacts and companies (0082). A manager writing one approves it by existing. Two independent mechanisms refuse a user's self-approval: this trigger, and the UPDATE policy's WITH CHECK.",
    "created"),
  e("report_templates.report_template_approved_by", "Signed off by",
    "Which manager put it in the library. An id, not a name: the screens that show it already hold listProfiles(), and an embed of profiles from here would be the PGRST201 ambiguity verify/embeds.sh exists to catch, because the audit quartet gives this table two foreign keys to profiles.",
    "uuid", "Nullable. FK → profiles(profile_id).", "—", "created"),
  e("report_templates.report_template_is_active", "In use",
    "Retired rather than deleted. A template that has been used is named by every document made from it, and deleting it would take that trail with it.",
    "boolean", "Not null, default true.",
    "The listing filters on it; the read policy does not, so a retired entry still resolves for the documents that name it.",
    "created"),

  e("report_documents.report_document_id", "Document",
    "One thing somebody made and is sending — a progress report, a client letter, a maintenance report.",
    "uuid", "Primary key, default gen_random_uuid().",
    "A COPY of a template rather than a view onto it. Amber, 4 Sep, on changing a letter's wording for one instance: if that edit wrote back to the template, the next person to use it would inherit one letter's wording, silently.",
    "created"),
  e("report_documents.report_document_title", "Title",
    "What the document is called, and what its exported file is named after.",
    "text", "Not null. CHECK: not blank after trimming.", "—", "created"),
  e("report_documents.report_document_layout", "The layout",
    "The same shape as a template's, and the same rule: references, never rows.",
    "jsonb", "Not null, default {\"widgets\": []}. CHECK: widgets is a JSON array.",
    "Diverges from its template the moment somebody edits it, which is the point of the copy.",
    "created"),
  e("report_documents.report_template_id", "Started from",
    "Which library template this was copied from, when it was copied from one. Null for a document written from scratch, and null again once that template is deleted.",
    "uuid", "Nullable. FK → report_templates(report_template_id) ON DELETE SET NULL.",
    "SET NULL rather than CASCADE: a letter that has already gone to a client does not vanish because somebody retired the template it was drafted from.",
    "created"),
  e("report_documents.job_id", "About which job",
    "The job this document is about. What the Record properties block resolves against when it is set to \"the record this document is about\" — which is what lets one template render 1042-001's properties on 1042-001 and 1043-002's on 1043-002 with nobody editing it.",
    "text", "Nullable. FK → jobs(job_id) ON DELETE CASCADE. CHECK: num_nonnulls(job_id, project_id) <= 1.",
    "CASCADE, unlike the template pointer: a document about a job that no longer exists is about nothing. At most one parent, and neither is also fine — a portfolio report is about the whole book of work.",
    "created"),
  e("report_documents.project_id", "About which project",
    "The project this document is about, on the same terms as job_id.",
    "integer", "Nullable. FK → projects(project_id) ON DELETE CASCADE. Same at-most-one check.",
    "—", "created"),
  e("report_documents.report_document_share_token", "Share link",
    "A way into this document for somebody with no login — Amber, 4 September: \"documents must be able to be sent outside of Lofty… by the share button\". 256 bits of CSPRNG, base64url, written by the Share panel.",
    "text", "Nullable. UNIQUE. Partial index on the non-null rows. CHECK: both-or-neither with the expiry, and null unless a snapshot exists.",
    "Deliberately not served by RLS: making this table readable by `anon` to answer a share link would expose every other document in it to anybody holding any link. It is a server endpoint with the service role, returning one row — and since 0095 that row is a stored snapshot, so the endpoint has no query to scope wrongly.",
    "created"),
  e("report_documents.report_document_share_expires_at", "Link expires",
    "When the share link stops working. Mandatory whenever there is a link, by constraint — a link that never expires outlives the reason it was made, and nobody ever goes back to revoke it.",
    "timestamptz", "Nullable, but null exactly when the token is null.",
    "The pair check is what makes \"forever\" unwritable rather than merely discouraged.",
    "created"),
  e("report_documents.report_document_share_password_hash", "Link password",
    "The hash of the optional password on a share link. Never leaves the server: the repository's column list does not include it, and the row type the browser gets carries hasSharePassword instead.",
    "text", "Nullable. CHECK: null unless there is a token.",
    "PBKDF2-SHA256, 210,000 rounds, derived IN THE BROWSER when the link is made (data/sharePassword.ts) and verified in the endpoint from the plaintext the viewer types. CORRECTS AN EARLIER NOTE HERE that said hashing in the browser protects nothing: that is true of hashing at VERIFICATION time, where the hash becomes the password. Deriving at CREATION time is the opposite — the plaintext never leaves the tab that typed it, and the endpoint still does the real work against what the viewer sends. `npm run check:share-password` imports both ends and asserts they agree.",
    "created"),
  e("report_documents.report_document_published_at", "Published on",
    "When the document was last saved into SharePoint. Null is the ordinary state and means DRAFT — which is what the watermark on screen, in Print/Save PDF, in the .html download and in the .docx is reading. Amber, 10 Sep: \"have a DRAFT watermark across it while editable and saved to the job\".",
    "timestamptz", "Nullable. CHECK: both-or-neither with report_document_published_by, and null unless report_document_published_url OR report_document_published_document_id is set (0110 widened this).",
    "Stamped by guard_report_document_publication() from the database clock, never typed — and CLEARED by the same trigger whenever the layout or title changes, because Amber: \"if editing it in the app it reverts to draft\". A trigger rather than a rule each caller keeps: the builder autosaves, the panel writes and the importer writes, and a per-caller promise is one the next caller forgets. Sharing or moving the document does NOT revert it — nothing a reader would see has changed. There is deliberately no status column: a word somebody sets would drift from the fact, and a draft wearing a published label is exactly what the watermark exists to prevent.",
    "created"),
  e("report_documents.report_document_published_by", "Published by",
    "Which person last sent it. An id, not a name: this table already has two foreign keys to profiles from the audit quartet, so an embed from here is the PGRST201 ambiguity verify/embeds.sh exists to catch.",
    "uuid", "Nullable. FK → profiles(profile_id) ON DELETE SET NULL.",
    "Read from the session by the guard rather than trusted from the argument, the same rule as every other \"who did this\" column here.", "created"),
  e("report_documents.report_document_published_url", "Published to",
    "The SharePoint address the document was saved at. Pasted by hand until the integration lands; the publish control pre-fills it from the record's own folder (jobs.job_sharepoint_url, 0040), which is Amber's \"should default to job file\".",
    "text", "Nullable. CHECK report_documents_published_url_is_https — https and no whitespace, deliberately loose.",
    "SURVIVES A REVERT, on purpose. A URL with no report_document_published_at is a real and readable state: this went to SharePoint, somebody has edited it since, and what is up there is out of date — the app shows that as its own chip. Keeping it means re-publishing pre-fills where it went last time rather than sending somebody back into SharePoint to find the address again. Loose because tenant paths, personal sites and shortened share links all look different and a pattern tight enough for one refuses the other two.",
    "created"),
  e("report_documents.report_document_published_document_id", "Copy saved on the record",
    "The file this document was published AS, held by Lofty and filed against the job or project it is about — the alternative to a SharePoint address, and allowed alongside one. Amber, 10 Sep: \"until Documents are integrated to Sharepoint, please allow the option of saving to Job in the system and/or downloading it and adding a link to that document file\".",
    "uuid", "Nullable. FK → documents(document_id) ON DELETE SET NULL. Either this or report_document_published_url must be set whenever report_document_published_at is.",
    "WHY A POINTER INTO documents RATHER THAN A PATH HERE: filing it in 0032's table is what makes the published copy appear in the record's own Documents list beside the contract and the survey, instead of in a place only this table knows about. The bytes go in the private job-documents bucket, read by signed URL — the opposite call to 0100's public report-images, because this is the work product rather than a picture inside it. SURVIVES A REVERT, exactly as the URL does, so re-publishing does not mean finding the file again. ONE COPY ONLY (0111): publishing again deletes the copy the previous publish saved, because Amber, 10 Sep — \"only onver version of the document. if they want another copy they can download it\". A copy somebody has since filed on another record is unpointed rather than deleted, which is the one carve-out. documents.supersedes_id is deliberately unused here: it answers \"and what did it replace\", and the answer was that the replaced one is not wanted. AND ON DELETE, THE PUBLICATION GOES WITH IT — but only when this was the only answer to \"where did it go\": documents_reap_publication takes the document back to draft, because a row still flying a published flag over a file that no longer exists is the drift the whole draft/published design exists to prevent. Written first as a bare ON DELETE SET NULL, which does NOT work: the nulling is an update, and it leaves a published row naming nothing, which the check refuses — so the admin\'s delete failed with a constraint message they could not act on.",
    "created"),
  e("report_documents.report_document_share_snapshot", "What was sent",
    "The document as it was when the link was made — the compiled model the PDF and Word writers also take. A shared link does NOT re-resolve: it is a sent document, not a live window into Lofty. Re-share to send newer numbers.",
    "jsonb", "Nullable. CHECK: report.sections must be an array. A token requires one; revoking does not clear it.",
    "This is the answer to \"what may somebody outside Lofty see of a job\", and it removes the question instead of answering it. The snapshot is compiled in the author's browser under their own RLS, so it holds only what they could see and chose to send — and the endpoint that serves it queries nothing, so there is no forgotten filter that could return the whole book of work.",
    "created"),
  e("report_documents.report_document_has_share_password", "Has a password",
    "Whether a password is set, for the Share panel to say so.",
    "boolean", "Generated always as (password_hash is not null), stored.",
    "Generated rather than maintained: a derived flag some code path forgets to update is how a cleared password goes on reading as set. It exists so the hash itself never has to be selected into a browser — 0095 revokes the column grant for both.",
    "created"),
  e("report_documents.report_document_has_share_snapshot", "Has been shared",
    "Whether a snapshot exists, so a list can say a document has been sent without loading it.",
    "boolean", "Generated always as (share_snapshot is not null), stored.",
    "Same reasoning as the password flag, plus size: the snapshot is a whole compiled document, and no signed-in screen renders one.",
    "created"),
...
export const TABLE_DESCRIPTIONS: Record<string, string> = {
  report_templates:
    "The template library behind Tools → Template Builder (0094): whole templates and the reusable sections dropped into them, at global, team or manager visibility. Anyone at user and above may write one; it is not in the library, and nobody else can see it, until a manager signs it off — the same approval shape contacts and companies have had since 0082. Blocks hold references to the app's data rather than copies, so a template renders current jobs whenever it is opened.",
  report_documents:
    "What somebody made from the library (0094) — a progress report, a client letter, a maintenance report. A copy, not a view: editing a document never writes back to its template, because a change made for one letter must not reach the next person to use it. Optionally about a job or a project, which is what the Record properties block reads. The share columns are inert as shipped; the endpoint that would serve them is written and not deployed.",
  activity:
