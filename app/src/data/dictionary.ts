...
export const DICTIONARY: DictionaryEntry[] = [
  ...
  e("addresses.address_res_number", "Res number",
    ...
    "text",
    "Nullable, and 0105's own note says why it is not forbidden on a project's address even though the app only offers it on a job's: an address row is not owned by one record, so there is nothing on it to hang \"this belongs to a job\" from. Text for the same reason the lot number is text, and Amber wrote \"(number)\" against both — \"2B\" is a real lot number and a res number is the same kind of label off the same plan. Not unique: res numbers repeat across sites by definition, and whether they may repeat within one has not been stated.",
    "Leads consolidated_address when set — \"Res 1, Lot 3, 13 Tester Street, Testville, SA, 5000\" (Amber's own example, 10 Sep, and one of 0105's five probes).",
    "created"),
  ...
  e("report_documents.report_document_published_at", "Published on",
    ...
    "timestamptz", "Nullable. CHECK: both-or-neither with report_document_published_by, and null unless report_document_published_url is set.",
