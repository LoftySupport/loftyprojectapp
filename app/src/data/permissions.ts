...
export const PERMISSION_RULES: readonly PermissionRule[] = [
  ...
  { object: "Jobs", action: "Pin or release a job's stage", needs: "manager",
    enforcedBy: "guard_job_stage_pin() trigger, 0132",
    note: "The override on the derivation, so it costs the same as the move it replaces. A column rule again, and the reason is part of the pin: a user who could rewrite it could change what the record says a manager decided." },
  { object: "Jobs", action: "Let a finished process move the job", needs: "user",
    enforcedBy: "guard_job_stage_change()'s one exception, 0132",
    note: "The derivation's own answer is not a person's decision, so it is not a person's permission. Anyone who may finish the work may let the work move the job — narrowly: the new stage must be exactly what job_derived_stage() computes, and the job must not be pinned." },
